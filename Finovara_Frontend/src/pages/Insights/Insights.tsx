              ...
              {displayed.map((article, i) => (
                <div key={article.id} className="group bg-white rounded-2xl overflow-hidden border border-[#E2E8F0] hover:shadow-xl transition-all hover:-translate-y-1">
                  <div className="h-48 overflow-hidden bg-[#EEF1F5]">
                    <img src={article.cover_image_url || (article as any).image || (i % 2 === 0 ? FALLBACK_IMG_A : FALLBACK_IMG_B)} alt={article.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  </div>
                  <div className="p-6">
