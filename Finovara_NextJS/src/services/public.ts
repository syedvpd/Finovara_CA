...
export interface Blog {
  ...
  category_id: string | null; published_at: string | null; read_time_minutes: number | null;
}
export interface Career {
  id: string; title: string; department: string | null; location: string | null;
  employment_type: string | null; description: string | null; is_open: boolean;
}
export interface Testimonial {
  id: string; author_name: string; author_title: string | null; content: string; rating: number | null;
}
...
export interface ContactPayload {
  ...
  phone?: string; subject?: string; service_id?: string; website?: string;
}
...
export const publicApi = {
  ...
  submitContact: (payload: ContactPayload) => api.post<{ message: string }>("/public/contact", payload),
  applyForVacancy: (careerId: string, fields: {
    applicant_name: string; applicant_email: string; applicant_phone: string;
    cover_letter?: string; resume: File;
  }) => {
